...
```{r ML application}
diabete_dat$Outcome <- factor(diabete_dat$Outcome, levels = c("0", "1"))
## The data name is diabete_dat 
set.seed(415)
K <- 10       ## Number of replicates 
n.case <- which(diabete_dat$Outcome==1)
n.control <- which(diabete_dat$Outcome==0)
...
RFresult <- lapply(train_rows, function(x){RF.boot.err.Func(train=diabete_dat[x, ], test=diabete_dat[-x, ], depVar = depVar, ntree=ntree)})
names(RFresult) <- paste0("RF_iter", 1:K)

PRresult <- lapply(train_rows, function(x){PR.boot.err.Func(x.train=diabete_dat[x, (names(diabete_dat)!=depVar)], y.train=diabete_dat[x, depVar], x.test=diabete_dat[-x, (names(diabete_dat)!=depVar)], y.test=diabete_dat[-x, depVar], alphaLevel=alphaLevel, family="binomial", type="class")})
names(PRresult) <- paste0("PR_iter", 1:K)

### SVM ###
SVMresult  <-  lapply(train_rows, function(x){SVM.boot.err.Func(train=diabete_dat[x, ], test=diabete_dat[-x, ], depVar= depVar, kernel="radial", cost=5)})
names(SVMresult) <- paste0("SVM_iter", 1:K)

### Neural Network ###
NNresult  <-  lapply(train_rows, function(x){NN.boot.err.Func(train=diabete_dat[x, ], test=diabete_dat[-x, ], depVar= depVar, hidden=10)})
...
GBMresult  <-  lapply(train_rows, function(x){GBM.boot.err.Func(train=diabete_dat[x, ], test=diabete_dat[-x, ], depVar= depVar, distribution="adaboost", n.trees = ntree)})
